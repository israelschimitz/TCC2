...
load C:\Israel\Engenharia_Elétrica\Disciplinas\TCC1\Desenvolvimento\ERDColetas\1204x128israels2.mat;
% load C:\Israel\Engenharia_Elétrica\Disciplinas\TCC1\Desenvolvimento\ERDColetas\2303x128israels.mat;

Ordem = Ordem1204x128israels2;
Dados = Touca1204x128israels2;
...
RC3Dir = sum(DadosDirC3MediaPot(1,1.5*SampleRate:2.5*SampleRate))/(SampleRate);
RC4Dir = sum(DadosDirC4MediaPot(1,1.5*SampleRate:2.5*SampleRate))/(SampleRate);
for k=1:(SampleRate*9)/TaxaSuavizacao
...
RC3Esq = sum(DadosEsqC3MediaPot(1,1.5*SampleRate:2.5*SampleRate))/(SampleRate);
RC4Esq = sum(DadosEsqC4MediaPot(1,1.5*SampleRate:2.5*SampleRate))/(SampleRate);
for k=1:(SampleRate*9)/TaxaSuavizacao
...
legend ('C3 (Hemisfério Esquerdo)','C4 (Hemisfério Direito)');
