...
%Touca0404x1000israels2.mat Touca0404x1000israels2 OrdemTouca0404x1000israels2
%% Carregando a coleta e a ordem
load C:\Israel\Engenharia_Elétrica\Disciplinas\TCC1\Desenvolvimento\ERDColetas\Touca0404x1000israels2.mat;
...
for i=1:QuantColetas
    if (mod(OrdemTouca0404x1000israels2(i),2) == 1)
        IndiceEsq(j,:) = i;
        j=j+1;
    else
        IndiceDir(k,:) = i;
...
for k=1:(QuantColetas/2)
    DadosDireita(:,:,k) = Touca0404x1000israels2(:,:,IndiceDir(k));
end
...
for k=1:(QuantColetas/2)
    DadosEsquerda(:,:,k) = Touca0404x1000israels2(:,:,IndiceEsq(k));
end
