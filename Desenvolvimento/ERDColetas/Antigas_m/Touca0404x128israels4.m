...
%Touca0404x128israels4.mat Touca0404x128israels4 OrdemTouca0404x128israels4
...
for i=1:QuantColetas
    if (mod(OrdemTouca0404x128israels4(i),2) == 1)
        IndiceEsq(j,:) = i;
        j=j+1;
    else
        IndiceDir(k,:) = i;
...
for k=1:(QuantColetas/2)
    DadosDireita(:,:,k) = Touca0404x128israels4(:,:,IndiceDir(k));
end
...
for k=1:(QuantColetas/2)
    DadosEsquerda(:,:,k) = Touca0404x128israels4(:,:,IndiceEsq(k));
end
...
RC3Dir = sum(DadosDirC3MediaPot(1,1*SampleRate:2*SampleRate))/(2*SampleRate);
RC4Dir = sum(DadosDirC4MediaPot(1,1*SampleRate:2*SampleRate))/(2*SampleRate);
for k=1:(SampleRate*9)/8
...
RC3Esq = sum(DadosEsqC3MediaPot(1,1*SampleRate:2*SampleRate))/(2*SampleRate);
RC4Esq = sum(DadosEsqC4MediaPot(1,1*SampleRate:2*SampleRate))/(2*SampleRate);
for k=1:(SampleRate*9)/8
