...
%2303x128israels.mat Coletas2303x128israels Ordem2303x128israels
...
for i=1:QuantColetas
    if (mod(Ordem2303x128israels(i),2) == 1)
        IndiceEsq(j,:) = i;
        j=j+1;
    else
        IndiceDir(k,:) = i;
...
for k=1:(QuantColetas/2)
    DadosDireita(:,:,k) = Coletas2303x128israels(:,:,IndiceDir(k));
end
...
for k=1:(QuantColetas/2)
    DadosEsquerda(:,:,k) = Coletas2303x128israels(:,:,IndiceEsq(k));
end
