...
%2303x4israels.mat Coletas2303x4israels Ordem2303x4israels
%% Carregando a coleta e a ordem
load C:\Israel\Engenharia_Elétrica\Disciplinas\TCC1\Desenvolvimento\ERDColetas\2303x4israels.mat;
...
for i=1:QuantColetas
    if (mod(Ordem2303x4israels(i),2) == 1)
        IndiceEsq(j,:) = i;
        j=j+1;
    else
        IndiceDir(k,:) = i;
...
for k=1:(QuantColetas/2)
    DadosDireita(:,:,k) = Coletas2303x4israels (:,:,IndiceDir(k));
end
...
for k=1:(QuantColetas/2)
    DadosEsquerda(:,:,k) = Coletas2303x4israels(:,:,IndiceEsq(k));
end
