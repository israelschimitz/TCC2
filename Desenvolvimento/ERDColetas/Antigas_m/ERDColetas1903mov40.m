...
load C:\Israel\Engenharia_Elétrica\Disciplinas\TCC1\Desenvolvimento\ERDColetas\Mov40_1903.mat;
...
for i=1:QuantColetas
    if (mod(Ordem1903Mov40israel(i),2) == 1)
        IndiceEsq(j,:) = i;
        j=j+1;
    else
        IndiceDir(k,:) = i;
...
for k=1:(QuantColetas/2)
    DadosDireita(:,:,k) = Coletas1903Mov40israel (:,:,IndiceDir(k));
end
...
for k=1:(QuantColetas/2)
    DadosEsquerda(:,:,k) = Coletas1903Mov40israel (:,:,IndiceEsq(k));
end
