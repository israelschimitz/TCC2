...
%2303x6israels.mat Coletas2303x6israels Ordem2303x6israels
%% Carregando a coleta e a ordem
load C:\Israel\Engenharia_Elétrica\Disciplinas\TCC1\Desenvolvimento\ERDColetas\2303x6israels.mat;
...
for i=1:QuantColetas
    if (mod(Ordem2303x6israels(i),2) == 1)
        IndiceEsq(j,:) = i;
        j=j+1;
    else
        IndiceDir(k,:) = i;
...
for k=1:(QuantColetas/2)
    DadosDireita(:,:,k) = Coletas2303x6israels (:,:,IndiceDir(k));
end
...
for k=1:(QuantColetas/2)
    DadosEsquerda(:,:,k) = Coletas2303x6israels(:,:,IndiceEsq(k));
end
