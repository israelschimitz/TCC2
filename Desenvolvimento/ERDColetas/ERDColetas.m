...
load C:\Israel\Engenharia_Elétrica\Disciplinas\TCC1\Desenvolvimento\ERDColetas\Mov160.mat;
...
for k=1:(QuantColetas/2)
    DadosDireita(:,:,k) = ColetasMov160 (:,:,IndiceDir(k));
end
...
for k=1:(QuantColetas/2)
    DadosDireitaC4(:,k) = DadosDireita(:,2,k); %canal 2 - C4
...
for k=1:(QuantColetas/2)
    DadosEsquerda(:,:,k) = ColetasMov160 (:,:,IndiceEsq(k));
end
...
% Análise C4 - Filtra (Banda Mu) e aplica a potência
for k=1:(QuantColetas/2)
    DadosEsquerdaC4(:,k) = DadosEsquerda(:,2,k); %canal 3 - C4
...
Titulo1=strcat('Sinal C3 bruto (Coleta ',int2str(OrdemMov160(Coleta)),') - Imaginação do movimento Mão Direita');
title(Titulo1);
xlabel('Tempo [s]'); 
ylabel('Tensão [V]');
subplot(4,2,3);
plot(Time,DadosDirC3FiltradosMu(IndiceDir(Coleta),:));
Titulo3=strcat('Sinal C3 Filtrado Mu 8-11 Hz (Coleta ',int2str(OrdemMov160(Coleta)),')');
title(Titulo3);
xlabel('Tempo [s]'); 
ylabel('Tensão [V]');
subplot(4,2,5);
plot(Time,DadosDirC3Potencia(IndiceDir(Coleta),:));
Titulo5=strcat('Sinal C3 elevado ao quadrado - Potência (Coleta ',int2str(OrdemMov160(Coleta)),')');
title(Titulo5);
xlabel('Tempo [s]'); 
ylabel('Potência [V^2]');
subplot(4,2,7);
...
Titulo2=strcat('Sinal C4 bruto (Coleta ',int2str(OrdemMov160(Coleta)),') - Imaginação do movimento Mão Direita');
title(Titulo2);
xlabel('Tempo [s]'); 
ylabel('Tensão [V]');
subplot(4,2,4);
plot(Time,DadosDirC4FiltradosMu(IndiceDir(Coleta),:));
Titulo4=strcat('Sinal C4 Filtrado Mu 8-11 Hz (Coleta ',int2str(OrdemMov160(Coleta)),')');
title(Titulo4);
xlabel('Tempo [s]'); 
ylabel('Tensão [V]');
subplot(4,2,6);
plot(Time,DadosDirC4Potencia(IndiceDir(Coleta),:));
Titulo6=strcat('Sinal C4 elevado ao quadrado - Potência (Coleta ',int2str(OrdemMov160(Coleta)),')');
title(Titulo6);
xlabel('Tempo [s]'); 
ylabel('Potência [V^2]');
subplot(4,2,8);
...
Titulo10=strcat('Sinal C3 bruto (Coleta ',int2str(OrdemMov160(Coleta)),') - Imaginação do movimento Mão Esquerda');
title(Titulo10);
xlabel('Tempo [s]'); 
ylabel('Tensão [V]');
subplot(4,2,3)
plot(Time,DadosEsqC3FiltradosMu(IndiceEsq(Coleta),:));
Titulo30=strcat('Sinal C3 Filtrado Mu 8-11 Hz (Coleta ',int2str(OrdemMov160(Coleta)),')');
title(Titulo30);
xlabel('Tempo [s]'); 
ylabel('Tensão [V]');
subplot(4,2,5)
plot(Time,DadosEsqC3Potencia(IndiceEsq(Coleta),:));
Titulo50=strcat('Sinal C3 elevado ao quadrado - Potência (Coleta ',int2str(OrdemMov160(Coleta)),')');
title(Titulo50);
xlabel('Tempo [s]'); 
ylabel('Potência [V^2]');
subplot(4,2,7)
...
Titulo20=strcat('Sinal C4 bruto (Coleta ',int2str(OrdemMov160(Coleta)),') - Imaginação do movimento Mão Esquerda');
title(Titulo20);
xlabel('Tempo [s]'); 
ylabel('Tensão [V]');
subplot(4,2,4);
plot(Time,DadosEsqC4FiltradosMu(IndiceEsq(Coleta),:));
Titulo40=strcat('Sinal C4 Filtrado Mu 8-11 Hz (Coleta ',int2str(OrdemMov160(Coleta)),')');
title(Titulo40);
xlabel('Tempo [s]'); 
ylabel('Tensão [V]');
subplot(4,2,6);
plot(Time,DadosEsqC4Potencia(IndiceEsq(Coleta),:));
Titulo60=strcat('Sinal C4 elevado ao quadrado - Potência (Coleta ',int2str(OrdemMov160(Coleta)),')');
title(Titulo60);
xlabel('Tempo [s]'); 
ylabel('Potência [V^2]');
subplot(4,2,8);
...
RC3Dir = sum(DadosDirC3MediaPot(1,500:2500))/(2*SampleRate);
RC4Dir = sum(DadosDirC4MediaPot(1,500:2500))/(2*SampleRate);
for k=1:(SampleRate*9)/8
...
RC3Esq = sum(DadosEsqC3MediaPot(1,500:2500))/(2*SampleRate);
RC4Esq = sum(DadosEsqC4MediaPot(1,500:2500))/(2*SampleRate);
for k=1:(SampleRate*9)/8
